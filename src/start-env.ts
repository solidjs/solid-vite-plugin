...
// The virtual modules bake the *validated output values* as plain JSON:
// the validator library never reaches any bundle, client bundles only ever
// see the client subset, and the values are build-time env by design (the
// server bundle carries its values too — rebuild to change them).
//
...
interface LoadedEnv {
  schema: EnvSchema;
  /** Every validated output value (server + client). */
...
export function startEnv(option: boolean | string | undefined): Plugin[] {
  ...
  async function loadAndValidate(): Promise<LoadedEnv> {
        ...
        if (result.issues && result.issues.length) {
          for (const issue of result.issues) {
            const at = (issue.path ?? [])
              ...
              .join('.');
            issues.push({ key: at ? `${key}.${at}` : key, message: issue.message });
          }
        } else {
    ...
    if (issues.length) {
      throw new Error(formatValidationError(issues, envFile, config.mode));
    }
  ...
  // validator code exists in any bundle and tree-shaking sees plain data.
  // `moduleType` marks the virtual source as plain JS for rolldown (Vite 8).
  ...
  return [
    {
      ...
      config(userConfig, env) {
        ...
        // Preview serves finished artifacts — values were baked at build
        // time, so no schema loading, validation, or fold happens there.
        if (!isPreview) resolveEnvFile();
      },
      ...
      async load(id, opts) {
        ...
        if (id === RESOLVED_SERVER_ENV_ID) {
          ...
          return envModuleCode(loaded.all);
        }
        ...
        if (leaks.length) {
          this.error(
            `[vite-plugin-solid] server env values leaked into client chunks:\n` +
              ...
              `\n\nServer vars are baked only into server bundles. Check for imports of ` +
