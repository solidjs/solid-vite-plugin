// Typed, validated environment variables as an app-mode feature (`app.env`):
...
function generateTypes(schema: EnvSchema, envFileAbs: string): void {
  ...
  const content =
    `// Generated by @solidjs/vite-plugin (app.env) — do not edit.\n` +
...
/**
 * App-mode typed env (the `app.env` option). Returns no plugin when the
 * feature is off (`env: false`, or nothing to probe); the feature is
 * app-scoped by construction: the option lives on `app`, so a bare
 * `ssr: true` setup has no env layer (documented).
 */
export function appEnv(option: boolean | string | undefined): Plugin[] {
  ...
  function resolveEnvFile(): void {
    if (typeof option === 'string') {
      ...
      if (!existsSync(absolute)) {
        throw new Error(`[@solidjs/vite-plugin] app.env does not exist: ${option}`);
      }
      const relative = path.relative(root, absolute).split(path.sep).join('/');
      if (relative.startsWith('..')) {
        throw new Error(
          `[@solidjs/vite-plugin] app.env must live inside the Vite root: ${option}`,
        );
      }
      envFileAbs = absolute;
      envFile = relative;
    ...
    if (option === true) {
      throw new Error(
        `[@solidjs/vite-plugin] app.env is enabled but no schema file was found: add ` +
          `${ENV_FILE_CANDIDATES.join(' or ')} at the project root (default-exporting ` +
          `{ server, client } maps of Standard Schema validators), or point app.env ` +
          `at a path.`,
      );
    }
  }
  ...
  function envModuleCode(values: Record<string, unknown>) {
    return {
      code:
        `// Generated by @solidjs/vite-plugin (app.env)\n` +
  ...
  function serverEnvModuleCode(loaded: LoadedEnv) {
    ...
    if (!serverKeys.length) {
      return {
        code:
          `// Generated by @solidjs/vite-plugin (app.env) — server env.\n` +
          `${baked}\n` +
