...
function HexChunk(props: HexViewerInnerProps) {
  ...
  return (
    <Text data-solid-hex-chunk options={{ size: 'xs', weight: 'bold', wrap: 'nowrap' }}>
      {content()}
    </Text>
  );
}
...
export function HexViewerInner(props: HexViewerInnerProps): JSX.Element {
  ...
  return (
    <div data-solid-hex-viewer>
      <div data-solid-hex-viewer-bytes>
        <For each={rows()}>{(current) => <HexRow bytes={current} />}</For>
      </div>
      <div data-solid-hex-viewer-text>
