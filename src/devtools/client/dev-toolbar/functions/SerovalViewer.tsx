...
function renderSerovalNode(
  ...
  if (
    ...
    return (
      <CascadeOption data-solid-seroval-link value={index}>
...
function SerovalNodeRenderer(props: SerovalNodeRendererProps): JSX.Element {
  ...
  return (
    <>
      <div data-solid-seroval-node>
        <div data-solid-seroval-node-header>
          <Badge type="info">{getNodeType(props.node)}</Badge>
          {props.node.i != null && <Badge type="info">{`id: ${props.node.i}`}</Badge>}
        </div>
        <div data-solid-seroval-node-content>{renderSerovalNode(props, props.node, onSelect)}</div>
      </div>
...
function SerovalRenderer(props: SerovalRendererProps): JSX.Element {
  const rest = omit(props, 'node');
  return (
    <div data-solid-seroval-renderer>
